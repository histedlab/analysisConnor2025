...
class CaimanRun:
    ...
    def __init__(self,path,fname="imageFrames.tif",caiman_opts_dict={},dview_opts_dict={}):
        """
        Args: 
        ...
        # update dview options if needed
        self.dview_default_opts = {
            'n_processes':None,
            'single_thread':False,
        }
        self.dview_default_opts.update(dview_opts_dict)
        ...
        self._out_mc_tif_name = os.path.join(self.basedir, "imageFrames-mc2.tif")

        # figs
        self._out_blank_fig_name = os.path.join(self.outdir, "fig-blank-stats.png")  # too many objs for pdf
        self._out_cnm1_fig_name = os.path.join(self.outdir, "cnmf-run1-contours.png")
        self._out_cnm2_fig_name = os.path.join(self.outdir, "cnmf-run2-contours.png")
        self._out_cnmf1_fig_name = os.path.join(self.outdir, "cnmf-run1-eval.png")
        self._out_cnmf2_fig_name = os.path.join(self.outdir, "cnmf-run2-eval.png")
        self._out_cnm_trace_fig_name = os.path.join(self.outdir, "trace_examples.png")
        ...
        out_caiman_opts_name = os.path.join(self.outdir, "caiman_params.txt")
        param_textfile = open(out_caiman_opts_name, 'wt')
        param_textfile.write(str(opts_ourdefaults_dict))
    ...
    def __enter__(self):
        ...
        if not hasattr(self, "dview"):
            ...
            c, self.dview, self.dview_n_processes = cm.cluster.setup_cluster(
                backend="local", 
                n_processes=self.dview_default_opts['n_processes'], 
                single_thread=self.dview_default_opts['single_thread']
            )
    ...
    def run_motion_correction(self, save_mc_as_tif=False, mc_template=None, use_original_tif_savename=False):
        """
        ...
        if save_mc_as_tif:
            ...
            if use_original_tif_savename:
                save_name_mc = os.path.join(self.basedir,self.base_fname[:-4]+'-mc.tif')
                tfl.imsave(save_name_mc, np.around(mc_mmap - min_mc_pixval).astype("uint16"))
            else:
    ...
    def load_motion_correct_mmap(self,save_mmap_to_tif=False,use_original_tif_savename=False):
        ...
        if save_mmap_to_tif:
            self._mc_mmap_name = self.mc_data.mmap_file[0]
            print(f'Loading mmap from {self._mc_mmap_name}...\n')
    ...
    def run_cnmf_twostep(self, extract_dff=False, cnmfRuns=1):
        ...
        plt.title("Contour plots of found components: n = "+str(idx_components.shape[0]+idx_components_bad.shape[0]))
        fig.set_size_inches(r_[1, 0.75] * 16)
        plt.savefig(self._out_cnm1_fig_name,bbox_inches='tight',dpi=600)
        ...
        plt.title("Contour plots of accepted components; n = "+str(idx_components.shape[0]))
        plt.subplot(122)
        crd_bad = cm.utils.visualization.plot_contours(cnme.A[:, idx_components_bad], Cn, vmax=0.75, **exargs)
        plt.title("Contour plots of rejected components; n = "+str(idx_components_bad.shape[0]))
        plt.savefig(self._out_cnmf1_fig_name,bbox_inches='tight',dpi=600)
    ...
    def delete_mmap(self):
        # delete mmap; use .tif instead to save on storage
        os.remove(self.mc_data.mmap_file[0])
    ...
    def _cnmf_make_trace_figures(self, cnmfRuns=1):
        ...
        plt.savefig(self._out_cnm_trace_fig_name,bbox_inches='tight')
