...
class CaimanRun:
    ...
    def run_motion_correction(self, save_mc_as_tif=False, mc_template=None, multiple_files=False):
        """
        ...
        if self.opts.get("motion", "pw_rigid"):
            ...
            ).astype(int) #switched np.int to int
        else:
        ...
        if save_mc_as_tif:
            ...
            if multiple_files:
                tfl.imsave(self._out_mc_tif_name[:-4]+self.fname[0][-10:], np.around(mc_mmap - min_mc_pixval).astype("uint16")) #change imageFrames-mc.tif to include the last 8 characters of infile fname if running multiple tifs through mc
            else:
