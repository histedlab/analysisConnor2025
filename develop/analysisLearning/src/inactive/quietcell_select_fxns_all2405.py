...
class analysis_2p_general:
    ...
    def generate_dfof_cell_traces_suite2p(self,file_path_masks,mwf, idxs=None):
        '''load fluorescence traces from suite2p, then calculate
        dF and dF/F values across all frames'''
        # set dF/F parameters
        nTrial = self.nTrialV*mwf.nstim
        nFrTrial = self.nFrTrial
        nPreFr = self.nPreFr
        nCell = self.nCell
        br = self.baseline_range
        ...
        F = F_all[accepted_cell_idxs][:,:nFrTrial*(nTrial)]
    ...
    def generate_dfof_quiet(self,traces_parsed,mwf,iVisStim=[0,1,2,3,4,5],i2PStim=[6,7,8,9],nStimFr_cut=2, n2PRespFr=10):
        ...
        df2P_cell = np.zeros(shape = (self.nCell, len(i2PStim)))
        dfoF2P_cell = np.zeros(df2P_cell.shape)
        dfV_cell = np.zeros(shape = (self.nCell, len(iVisStim)))
        dfoFV_cell = np.zeros(dfV_cell.shape)
        
        avgTrial_cell = traces_parsed.mean(axis=2) #returns shape nCells, nLevelss, trialframes
        avgPrestim = np.mean(avgTrial_cell[:, :, br[0]:br[1]], axis=2) #returns shape nCells, nLevels
        avgStim2P = np.mean(avgTrial_cell[:, i2PStim, br[1]+nStimFr_cut:br[1]+nStimFr_cut+n2PRespFr], axis=2)#returns shape nCells, nLevels
        avgStimV = np.squeeze(np.mean(avgTrial_cell[:, iVisStim, br[1]:br[1]+nStimFrV], axis=2)) #returns shape nCells, nLevels

        df2P_cell = avgStim2P - avgPrestim[:,i2PStim]
        dfoF2P_cell = df2P_cell / (avgPrestim[:,i2PStim])*100
        dfV_cell = avgStimV - avgPrestim[:,iVisStim]
        dfoFV_cell = dfV_cell / (avgPrestim[:,iVisStim])*100
    ...
    def plot_dfof_map_cell(self,aip,cell_masks,dfoF_cell,color_lim=50,save_path=None,save=False):
        '''Compute a single trial averaged dF/F map value for each cell mask of repeated 2p stimulation
        Inputs:
        ...
        for stim in range(nStims):
            plt.figure(figsize=[12,9])
            for iC in range(nCell):
                plt.imshow(dFoF_map_cell_masks[iC,stim,:,:],cmap=cmap,vmin=-v,vmax=v)
...
class analysis_vis_and_other:
    ...
    def parse_mwk_levels(self,mwf):
        ...
        for (iL,tL) in enumerate(levels):
            desIdx = np.where(mwf.stimDf[mwf.levelVar].values==tL)[0]
            #print(len(desIdx)) #code for when stims get cut (Corrupt file error) 
            #if tL==8: #code for when stims get cut (Corrupt file error) 
            #    desIdx = np.append(desIdx, 329) #code for when stims get cut (Corrupt file error) 
            levelFr_idx = []
            for idx in desIdx:
    ...
    def reshape_celltrace_by_mwk_levels(self,traces,mwf,nPlane=1):
        ...
        levels,levelFr,nLevel = self.parse_mwk_levels(mwf)
    ...
    def parse_trials_mwk(self,im_stim,aip,mwf):
        ...
        for trialType in range(0, nLevels):
            ...
            #grab those frames
            frames[trialType, :, :, :, :] = im_stim[[frame_num], :, :].reshape(trials, trialFrames, side_length, side_length) #reshape removes unneccesary 6th dim (length=1)
        
        #frames = frames-absMin
        
        return levelFrDir, frame_num, frames, absMin
    
    def dfof_pre_2P(self,frames,iTrial2P,smoothVal=3,nCutFrs2P=2,nRespFr=9):

        bl_range = self.baseline_range
        nHoloFr = self.nHoloFr
        
        avgTrial2P = np.mean(frames[iTrial2P, :, :, :, :], axis=1) #mean across trials avgtrial is [trialType, trialLen, sidelen, sidelen]
        avgPrestim2P = np.mean(avgTrial2P[:,bl_range[0]:bl_range[1], :, :], axis=1)
        avgStim2P = np.mean(avgTrial2P[:,bl_range[1]+nCutFrs2P:bl_range[1]+nCutFrs2P+nRespFr, :, :], axis=1)
    ...
    def dfof_pre_vis(self,frames,iTrialVis,smoothVal=3):
        ...
        avgTrialV = np.mean(frames[iTrialVis, :, :, :, :], axis=1) #mean across trials avgtrial is [trialType, trialLen, sidelen, sidelen]
        avgPrestimV = np.mean(avgTrialV[:,bl_range[0]:bl_range[1], :, :], axis=1)
        avgStimV = np.mean(avgTrialV[:,bl_range[1]:bl_range[1]+nStimFrV, :, :], axis=1)
