...
def check_nplanes(track_ops):
    ...
    for ds_path in track_ops.all_ds_path:
        ...
        n_planes = len([name for name in os.listdir(ds_path) if name.startswith('plane')])
...
def load_all_imgs(track_ops):
    ...
    for ds_path in track_ops.all_ds_path:
        ...
        for i in range(track_ops.nplanes):
            ops = np.load(ds_path + '/plane' + str(i) + '/ops.npy', allow_pickle=True).item()
...
def load_all_ds_stat_iscell(track_ops):
    ...
    for (i, ds_path) in enumerate(track_ops.all_ds_path):
        ...
        for j in range(track_ops.nplanes):
            stat = np.load(os.path.join(ds_path, f'plane{j}', 'stat.npy'), allow_pickle=True)
            iscell = np.load(os.path.join(ds_path, f'plane{j}', 'iscell.npy'), allow_pickle=True)
...
def load_all_ds_ops(track_ops):
    ...
    for ds_path in track_ops.all_ds_path:
        ds_ops = []
        for j in range(track_ops.nplanes):
            ops = np.load(os.path.join(ds_path, f'plane{j}', 'ops.npy'), allow_pickle=True).item()
            ds_ops.append(ops)
