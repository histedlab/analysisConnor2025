...
class CaimanRun:
    ...
    def run_motion_correction(self, save_mc_as_tif=False, mc_template=None, multiple_tifs=False):
        """
        ...
        if save_mc_as_tif:
            ...
            if multiple_tifs==False:
                tfl.imsave(self._out_mc_tif_name, np.around(mc_mmap - min_mc_pixval).astype("int16"))
                #tfl.imsave(self._out_mc_tif_name, np.around(mc_mmap - min_mc_pixval).astype("uint16"))
                print("Nina: bc of that time with paul and the affine im changing this to a signed integer")
            else:
                #file_id = str.split(self.fname[0],sep='_')
                #print(file_id[-1][:-4]) # the last bit of the string, minus the last 4 letters aka ".tif"
                tfl.imsave(os.path.join(self.outdir,'imageFrames-mc_'+self.fname[0][-9:-4]+'.tif'), np.around(mc_mmap - min_mc_pixval).astype("int16"))
                #tfl.imsave(os.path.join(self.outdir,'imageFrames-mc_'+self.fname[0][-9:-4]+'.tif'), np.around(mc_mmap - min_mc_pixval).astype("uint16"))
